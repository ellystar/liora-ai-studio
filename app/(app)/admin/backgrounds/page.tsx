...
export default function BackgroundsAdmin() {
  ...
  async function handleAdd() {
    ...
    setSaving(true)
    try {
      const thumbnail_url = await uploadAsset('backgrounds', file)
      const { error: insErr } = await supabase.from('backgrounds').insert({ name, thumbnail_url, prompt })
  ...
  return (
    <div>
      <div className="rounded-2xl border border-[#242424] bg-[#141414] p-5">
        <p className="mb-4 text-sm font-medium text-neutral-100">Yeni arkaplan ekle</p>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {items.map((b) => (
          <div key={b.id} className="overflow-hidden rounded-xl border border-[#242424] bg-[#141414]">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={b.thumbnail_url} alt={b.name} className="aspect-[3/4] w-full object-cover" />
            <div className="p-2">
              <div className="flex items-center justify-between">
                <p className="text-xs text-neutral-200">{b.name}</p>
                <button onClick={() => handleDelete(b.id)} aria-label="Sil" className="text-neutral-500 hover:text-red-400">
                  <Trash2 className="h-3.5 w-3.5" />
                </button>
              </div>
              <p className="mt-1 line-clamp-2 text-[10px] text-neutral-500">{b.prompt}</p>
            </div>
          </div>
        ))}
      </div>
